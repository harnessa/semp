...
def refl_planar(theta, kz_2d):
    ...
    # k=mp.Vector3(math.cos(theta), math.sin(theta)).scale(2*np.pi*fcen)
    ...
    sources = [mp.Source(mp.GaussianSource(fcen,fwidth=0.2*fcen, is_integrated=True),
                         component=src_comp,
                         center=mp.Vector3(-0.5*sx+dpml), size=mp.Vector3(y=sx),)]
                         # amp_func=pw_amp(k, mp.Vector3(x=-0.5*sx+dpml)))]
    ...
    vol = mp.Volume(size=mp.Vector3(npsx, npsx))
    ...
    sim.run(until_after_sources=mp.stop_when_fields_decayed(50, src_comp, mp.Vector3(-0.5*sx+dpml), 1e-9))
