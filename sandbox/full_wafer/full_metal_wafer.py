...
def run_sim(pol, is_vac, get_meta, w_support):

    #Print
    if mp.am_master():
        print(f'\nRunning: {pol}, vac: {is_vac}, support: {w_support}\n')

    #Get support wafer thickness
    if w_support:
        support_thick = support_thick_0
        sup_ext = ''
    else:
        support_thick = 0
        sup_ext = '_nosupport'
    ...
    #Directory
    save_dir = f'/home/aharness/Research/Optics_Modeling/Semp_Results/full_wafer/{session}{sup_ext}'
    # save_dir = f'/scratch/network/aharness/Semp_Results/full_wafer/{session}{sup_ext}'
...
for pol in ['s','p']:
    for is_vac in [False, True]:
        for is_support in [False, True]:
            ...
            #Run sim
            run_sim(pol, is_vac, get_meta, is_support)
